...
class Couchbase extends NoSQL {
  ...
  _connect(settings, database) {
    ...
    // For couchbase 5
    if (settings.auth) {
      // https://developer.couchbase.com/documentation/server/5.0/sdk/nodejs/start-using-sdk.html
      this._cluster.authenticate(settings.auth.name, settings.auth.password);
      bucketGen = this._cluster.call('openBucketAsync');
    } else {
      ...
      bucketGen = this._cluster.call('openBucketAsync', settings.bucket.name, settings.bucket.password);
    }
